...
gridContainer.childNodes.forEach(function(element) {
    ...
    // RESIZE THE BLOODY GRID!!
resizeButton.addEventListener('click', function(e) { 

    let newGridWidthPrompt = prompt("What is the width of the new grid? (Enter a whole number between 1 and 100): ")

    let newGridWidth = parseInt(newGridWidthPrompt);

    if (Number.isInteger(newGridWidth) === false) {
        alert("Follow the instructions please :) I'm setting the grid width to 10 until you do :)")
        newGridWidth = 10;
    }
    if (newGridWidth > 100) {
        alert("Oops, number too high! I'll just go ahead and set grid width to 100.")
        newGridWidth = 100;
    }
    if (newGridWidth < 1) {
    ...
    resetButton.addEventListener('click', function(e) { 
        ...
        for (let i = 0; i < resetGridSize; i++) {
            gridContainer.style.gridTemplateColumns = `repeat(${Math.sqrt(resetGridSize)}, 1fr)`
